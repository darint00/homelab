...
variable "vm_name_prefix" {
  description = "VM name prefix (VMs will be named <prefix>-node1, <prefix>-node2, etc.)"
  type        = string
  default     = "dc1"
}
...
variable "node_count" {
  description = "Total number of cluster nodes (node1 = controlplane, node2..N = workers)"
  type        = number
  default     = 3
...
variable "base_ip" {
  description = "Static IP of node1; subsequent nodes increment the last octet by 1"
  type        = string
  default     = "192.168.86.130"
}

variable "base_vmid" {
  description = "Proxmox VMID for node1; subsequent nodes increment by 1"
  type        = number
  default     = 320
}

variable "bootstrap_endpoints" {
  description = "Map of node name to DHCP address for initial Talos config apply. Populated by cluster.sh; leave empty."
  type        = map(string)
  default     = {}
}
