# ──────────────────────────────────────────────
# dc1 — 3-node Talos cluster (dc1-node1/2/3)
# ──────────────────────────────────────────────
...
locals {
  ...
  # node1 = controlplane, node2..N = workers
  nodes_map = {
  ...
  cluster_endpoint = "https://${local.nodes_map["node1"].ip}:6443"

  # Use DHCP bootstrap endpoint for initial config apply; fall back to static IP
  apply_endpoints = {
...
resource "talos_machine_bootstrap" "cluster" {
  client_configuration = talos_machine_secrets.cluster.client_configuration
  node                 = local.nodes_map["node1"].ip
  endpoint             = local.nodes_map["node1"].ip
